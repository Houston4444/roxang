...
def getSfzFileName(tone):
    return parameters['SFZ_FILE'].replace('${KEY_TONE}', tone)

def writeSfzFile(sfz_file, contents):
...
for n in range(12):
    ...
    if one_channel_per_tone:
        if n == 11:
            writeSfzFile(getSfzFileName(key_tone), contents)
        else:
            continue
    elif "${KEY_TONE}" in parameters["SFZ_FILE"]:
        writeSfzFile(getSfzFileName(keyTonePlus(n)), contents)
    else:
        writeSfzFile(getSfzFileName(key_tone), contents)
